...
public class StudentMinHeapTest {
    ...
    public void testSortEmpty() {
        assertEquals(new Integer[]{}, heap.heapsort(new Integer[]{}));
    }
    ...
    private void verifyHeap(Integer[] expected) {
        ...
        for (int i = 0; i < original.length; i++) {
            if (2 * i + 1 < original.length && original[i].compareTo((Integer) original[2 * i + 1]) > 0) isHeap = false;
            if (2 * i + 2 < original.length && original[i].compareTo((Integer) original[2 * i + 2]) > 0) isHeap = false;
        }
